...
async function showProduct(env:Env,chat:number,mid:number,pid:number){
  ...
  if(!p||!p.enabled)return edit(env,chat,mid,'❌ محصول پیدا نشد.',[[{text:'⬅️ فروشگاه',callback_data:'shop:list'}]]);
  ...
  const min=Math.max(1,Number(p.min_qty||1)),max=Math.max(min,Number(p.max_qty||10));const available=mode==='manual'?max:Math.min(max,Number(p.stock||0));
...
async function showOrderSummary(env:Env,uid:number,chat:number,mid:number,pid:number,qty:number,discountId=0){
  const access=await getUserAccess(env,uid);const p:any=await productWithMeta(env,pid);if(!p||!p.enabled)return edit(env,chat,mid,'❌ محصول در دسترس نیست.',[[{text:'⬅️ فروشگاه',callback_data:'shop:list'}]]);
  const min=Math.max(1,Number(p.min_qty||1)),max=Math.max(min,Number(p.max_qty||10)),mode=String(p.delivery_mode||'stock');if(!Number.isInteger(qty)||qty<min||qty>max||(mode!=='manual'&&qty>Number(p.stock||0)))return edit(env,chat,mid,'❌ تعداد یا موجودی محصول معتبر نیست.',[[{text:'⬅️ محصول',callback_data:`shop:p:${pid}`}]]);
...
async function buyProduct(env:Env,uid:number,chat:number,mid:number,pid:number,qty:number,discountId=0){
  ...
  const p:any=await productWithMeta(env,pid);if(!p||!p.enabled)return edit(env,chat,mid,'❌ محصول در دسترس نیست.',[[{text:'⬅️ فروشگاه',callback_data:'shop:list'}]]);
  const min=Math.max(1,Number(p.min_qty||1)),max=Math.max(min,Number(p.max_qty||10)),mode=String(p.delivery_mode||'stock');if(!Number.isInteger(qty)||qty<min||qty>max)return edit(env,chat,mid,'❌ تعداد نامعتبر است.',[[{text:'⬅️ محصول',callback_data:`shop:p:${pid}`}]]);
  ...
  if(mode!=='manual'&&!(await stockKey(env)))return edit(env,chat,mid,'⚠️ فروشگاه موقتاً برای تحویل امن استوک تنظیم نشده است. لطفاً با پشتیبانی تماس بگیر.',[[{text:'⬅️ فروشگاه',callback_data:'shop:list'}]]);
  ...
  if(mode!=='manual')statements.push(env.DB.prepare("UPDATE orders SET status='completed',completed_at=CURRENT_TIMESTAMP WHERE public_id=?").bind(oid));
  statements.push(env.DB.prepare("DELETE FROM security_guards WHERE tag IN (?,?,?)").bind(guardBalance,guardStock,guardDiscount));
  try{await env.DB.batch(statements);}catch(e){console.log('atomic purchase rejected',oid,String(e));const b=await balance(env,uid);const stock:any=await env.DB.prepare("SELECT COUNT(*) c FROM product_stock WHERE product_id=? AND status='available'").bind(pid).first();const msg=b.total+1e-9<total?`❌ موجودی اعتبار کافی نیست.\n\nنیاز: <b>${total.toFixed(2)}</b>\nموجودی: <b>${b.total.toFixed(2)}</b>`:(mode!=='manual'&&Number(stock?.c||0)<qty)?'❌ موجودی محصول در همین لحظه تغییر کرد.':'❌ سفارش هم‌زمان تغییر کرد یا کد تخفیف دیگر معتبر نیست. دوباره تلاش کن.';return edit(env,chat,mid,msg,[[{text:'🔄 تلاش دوباره',callback_data:`shop:summary:${pid}:${qty}:0`}],[{text:'⬅️ فروشگاه',callback_data:'shop:list'}]]);}
  ...
  if(mode!=='manual'){const remain:any=await env.DB.prepare("SELECT COUNT(*) c FROM product_stock WHERE product_id=? AND status='available'").bind(pid).first();const low=Number(p.low_stock_threshold??await getSetting(env,'low_stock_threshold','5'))||5;if(Number(remain?.c||0)<=low)await notifyAdmins(env,'low_stock',`⚠️ <b>موجودی کم</b>\n\n📦 ${esc(p.title)}\nموجودی باقی‌مانده: <b>${Number(remain?.c||0)}</b>`,String(pid));}
  await edit(env,chat,mid,`${mode==='manual'?'🕓 <b>سفارش ثبت شد</b>':'✅ <b>سفارش تکمیل شد</b>'}\n\n🧾 ${oid}\n🛍 ${esc(p.title)} × ${qty}\n${payText}\n\n📦 <b>تحویل:</b>\n${delivered}`,[[{text:'🧾 جزئیات سفارش',callback_data:`order:view:${order?.id}`}],[{text:'🆘 مشکل با این سفارش',callback_data:`support:order:${order?.id}`}],[{text:'🏠 منوی اصلی',callback_data:'menu'}]]);
}
...
async function productWithMeta(env:Env,pid:number){
  return env.DB.prepare("SELECT p.*,c.title category_title,c.emoji category_emoji,d.title delivery_title,d.emoji delivery_emoji,d.mode delivery_mode,d.template_text delivery_template,d.eta_text delivery_eta,(SELECT COUNT(*) FROM product_stock s WHERE s.product_id=p.id AND s.status='available') stock FROM products p LEFT JOIN shop_categories c ON c.id=p.category_id LEFT JOIN delivery_types d ON d.id=p.delivery_type_id WHERE p.id=?").bind(pid).first() as Promise<any>;
}
...
async function adminLowStock(env:Env,chat:number,mid:number){const global=Number(await getSetting(env,'low_stock_threshold','5'))||5;const q:any=await env.DB.prepare("SELECT p.id,p.title,p.low_stock_threshold,(SELECT COUNT(*) FROM product_stock s WHERE s.product_id=p.id AND s.status='available') stock FROM products p LEFT JOIN delivery_types d ON d.id=p.delivery_type_id WHERE p.enabled=1 AND COALESCE(d.mode,'stock')<>'manual' ORDER BY stock ASC,p.id DESC").all();const items=(q.results||[]).filter((x:any)=>Number(x.stock)<=Number(x.low_stock_threshold??global)).slice(0,25);const rows:Btn[][]=items.map((p:any)=>[{text:`⚠️ ${p.title} — ${p.stock}`,callback_data:`admin:product:${p.id}`}]);rows.push([{text:'⬅️ مرکز مدیریت',callback_data:'admin:home'}]);await edit(env,chat,mid,items.length?'⚠️ <b>موجودی‌های کم</b>':'✅ <b>موجودی کم نداریم.</b>',rows);}
...
async function showAdmin(env:Env,chat:number,mid?:number){
  const users:any=await env.DB.prepare('SELECT COUNT(*) c FROM users').first();const pending:any=await env.DB.prepare("SELECT COUNT(*) c FROM payment_invoices WHERE status IN ('pending','expired','awaiting_receipt','manual_review')").first();const tickets:any=await env.DB.prepare("SELECT COUNT(*) c FROM support_tickets WHERE status='open'").first();const today:any=await env.DB.prepare("SELECT COUNT(*) c,COALESCE(SUM(total_price),0) s FROM orders WHERE date(created_at)=date('now')").first();const global=Number(await getSetting(env,'low_stock_threshold','5'))||5;const lowq:any=await env.DB.prepare("SELECT p.low_stock_threshold,(SELECT COUNT(*) FROM product_stock s WHERE s.product_id=p.id AND s.status='available') stock FROM products p LEFT JOIN delivery_types d ON d.id=p.delivery_type_id WHERE p.enabled=1 AND COALESCE(d.mode,'stock')<>'manual'").all();const low=(lowq.results||[]).filter((x:any)=>Number(x.stock)<=Number(x.low_stock_threshold??global)).length;const lastScan=await getSetting(env,'scanner_last_run','—');
...
async function processBroadcasts(env:Env){
  ...
  for(const u of arr){last=u.id;let r:any;if(b.message_type==='photo'&&b.file_id)r=await tg(env,'sendPhoto',{chat_id:u.telegram_id,photo:b.file_id,caption:esc(String(b.text||'')),parse_mode:'HTML'});else if(b.message_type==='document'&&b.file_id)r=await tg(env,'sendDocument',{chat_id:u.telegram_id,document:b.file_id,caption:esc(String(b.text||'')),parse_mode:'HTML'});else r=await send(env,u.telegram_id,esc(String(b.text||'')));if(r.ok)sent++;else failed++;}
...
async function adminFeatures(env:Env,chat:number,mid:number){
  const keys=[['feature_shop','🛍 فروشگاه'],['feature_crypto','🪙 کریپتو'],['feature_card','🏦 کارت'],['feature_referral','🎁 Referral'],['feature_support','🆘 پشتیبانی'],['feature_broadcast','📣 Broadcast'],['feature_favorites','⭐ علاقه‌مندی'],['feature_restock_watch','🔔 اعلان موجودی'],['show_stock_to_users','📦 نمایش موجودی به کاربر']];const rows:Btn[][]=[];
...
async function handleAdminState(env:Env,m:Message,state:string){
  ...
  if(state.startsWith('admin_product_edit:')){const x=state.split(':');const pid=Number(x[1]),field=x[2];let ok=true;if(field==='title'){ok=!!text&&text.length<=200;if(ok)await env.DB.prepare('UPDATE products SET title=? WHERE id=?').bind(text,pid).run();}else if(field==='description')await env.DB.prepare('UPDATE products SET description=? WHERE id=?').bind(text.toUpperCase()==='OFF'?null:text.slice(0,5000),pid).run();else if(field==='price'||field==='cost'){const n=Number(text);ok=Number.isFinite(n)&&n>=0&&n<=1e6;if(ok)await env.DB.prepare(`UPDATE products SET ${field==='price'?'price_credits':'cost_credits'}=? WHERE id=?`).bind(n,pid).run();}else if(field==='warranty'||field==='format'){await env.DB.prepare(`UPDATE products SET ${field==='warranty'?'warranty_text':'format_text'}=? WHERE id=?`).bind(text.toUpperCase()==='OFF'?null:text.slice(0,1000),pid).run();}else if(field==='qty'){const m=text.match(/^(\d+)\s+(\d+)$/);ok=!!m&&Number(m[1])>=1&&Number(m[2])>=Number(m[1])&&Number(m[2])<=100;if(ok)await env.DB.prepare('UPDATE products SET min_qty=?,max_qty=? WHERE id=?').bind(Number(m![1]),Number(m![2]),pid).run();}else if(field==='low'){if(text.toUpperCase()==='OFF')await env.DB.prepare('UPDATE products SET low_stock_threshold=NULL WHERE id=?').bind(pid).run();else{const n=Number(text);ok=Number.isInteger(n)&&n>=0&&n<=100000;if(ok)await env.DB.prepare('UPDATE products SET low_stock_threshold=? WHERE id=?').bind(n,pid).run();}}else if(field==='sort'){const n=Number(text);ok=Number.isInteger(n)&&Math.abs(n)<=1e6;if(ok)await env.DB.prepare('UPDATE products SET sort_order=? WHERE id=?').bind(n,pid).run();}if(!ok){await send(env,m.chat.id,'❌ مقدار معتبر نیست.');return true;}await env.DB.prepare('UPDATE users SET state=NULL WHERE telegram_id=?').bind(uid).run();await logAdmin(env,uid,'product_edit','product',String(pid),field);const mm=await send(env,m.chat.id,'✅ ذخیره شد.');await adminProductDetail(env,m.chat.id,mm.result.message_id,pid);return true;}
...
async function handleWebAdmin(req:Request,env:Env,url:URL):Promise<Response|null>{
  ...
  if(req.method==='POST'){
    ...
    if(action==='feature_toggle'){const key=String(fd.get('key')||'');const allowed=new Set(['feature_shop','feature_crypto','feature_card','feature_referral','feature_support','feature_broadcast','feature_favorites','feature_restock_watch','show_stock_to_users']);if(!allowed.has(key))return new Response('invalid feature',{status:400});await setSetting(env,key,(await featureEnabled(env,key,true))?'0':'1');return Response.redirect(`${url.origin}/admin-web?tab=operations`,303);}
...
export default {
  ...
  async scheduled(_controller:ScheduledController,env:Env,ctx:ExecutionContext){
    ctx.waitUntil((async()=>{
      ...
      await migratePlainStock(env); await refreshRates(env); await scanPendingCrypto(env); await setSetting(env,'scanner_last_run',nowIso()); await processBroadcasts(env);
    })());
  }
};
